...
colour_palets = {'tree': ['green2', 'green3', 'green4', 'OliveDrab1', 'OliveDrab2', 'OliveDrab4',
                          'PaleGreen1', 'PaleGreen2', 'PaleGreen3','PaleGreen4'],
                 'rock': ['AntiqueWhite1','AntiqueWhite2','AntiqueWhite3','AntiqueWhite4']}

def connecting_key(origin, direction_key):
    return tuple(origin[x] + directions[direction_key][x] for x in (0,1))
...
class Rock(Location):
    def __init__(self):
        Location.__init__(self, 'Rock')
        self.tags.add('Impassable')
        self.tile = 'Grey'
        self.gen_bias.update({'Grass': 5, 'Rock': 1, 'Sand': 0.5})

class Water(Location):
    def __init__(self):
        Location.__init__(self, 'Water')
        self.tags.add('Impassable')
        self.tile = 'Blue'
        self.gen_bias.update({'Grass': 0.2, 'Water': 0.5, 'Sand': 0.3})

class Sand(Location):
    def __init__(self):
        Location.__init__(self, 'Sand')
        self.tags.add('Law')
        self.tile = 'Yellow'
        self.gen_bias.update({'Rock': 0.3, 'Sand': 2})


location_types = {'Grass': Grass, 'Rock': Rock, 'Water': Water, 'Sand':Sand}

location_quota = {'Grass':[10000,10000], 'Rock': [1200,1200], 'Sand':[2000,2000]}
location_size = {'Grass':(5,200), 'Rock': (1,20), 'Sand':(5,200), 'Water':(1,100)}
...
class World(object):
    ...
    def avaliable_features(self, terrain):
        return {feature_key: feature for feature_key, feature in }
